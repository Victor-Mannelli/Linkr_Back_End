...
export async function postPost(req, res) {
	...
	try {
		...
		trends.forEach(async (iten) => {
			const trendExist = await connection.query(
				"SELECT * FROM trending WHERE trending_name = $1;",
				[iten]
			);
			...
			if (trendExist.rowCount > 0) {
				...
				await connection.query(
					"UPDATE trending SET count = $1 WHERE id = $2;",
					[count, trendId]
				);
				await connection.query(
					"INSERT INTO post_trending (trending_id, post_id) VALUES ($1, $2);",
					[trendId, idPost]
				);
			} else {
